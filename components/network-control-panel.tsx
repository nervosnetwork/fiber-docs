...
// Scale: multiply meters by pixels per meter
// Canvas: 1200x616px, Node range: X(1.12-32.99m), Y(1.34-22.52m)
// Max scale by height: 616/22.52 ≈ 27.4, leaving margin: 27.0
const SCALE_TABLET = 24.0; // pixels per meter for tablet
const SCALE_DESKTOP = 27.0; // pixels per meter for desktop (balanced for 1200x616 canvas)

// Offset to center the network graph better in canvas
const OFFSET_X_DESKTOP = 200; // Move right 120px
const OFFSET_Y_DESKTOP = -15; // Move down 30px
const OFFSET_X_TABLET = 300; // Move right 80px for tablet
const OFFSET_Y_TABLET = 20; // Move down 20px for tablet

export default function NetworkControlPanel() {
  const [selectedMode, setSelectedMode] = useState<'open' | 'close' | null>(
    null
  );
  const [selectedNodes, setSelectedNodes] = useState<number[]>([]);
  const [customChannels, setCustomChannels] = useState<CustomChannel[]>([]);
  const [selectedChannel, setSelectedChannel] = useState<CustomChannel | null>(
    null
  );
  const [selectedEdge, setSelectedEdge] = useState<Edge | null>(null); // Track selected static edge
  const [closedEdges, setClosedEdges] = useState<Set<string>>(new Set()); // Track closed edges
  const [hoveredNodeId, setHoveredNodeId] = useState<number | null>(null);
  const [transaction, setTransaction] = useState<Transaction | null>(null);
  const [isTransacting, setIsTransacting] = useState(false);
  const [showChannelNotification, setShowChannelNotification] = useState(false);
  const [showChannelClosedNotification, setShowChannelClosedNotification] =
    useState(false);
  ...
  // Calculate nodes based on current scale and offset
  const nodes: Node[] = useMemo(() => {
    return [
      // Left side nodes - reduced offset to keep them on the left
      { id: 1, x: 1.12 * SCALE + OFFSET_X - 160, y: 5.53 * SCALE + OFFSET_Y }, // 左侧折线起点
      { id: 2, x: 3.96 * SCALE + OFFSET_X - 160, y: 3.91 * SCALE + OFFSET_Y }, // 左侧折线波峰
      { id: 3, x: 5.22 * SCALE + OFFSET_X - 160, y: 5.53 * SCALE + OFFSET_Y }, // 左侧折线波谷
      { id: 4, x: 9.65 * SCALE + OFFSET_X - 160, y: 2.59 * SCALE + OFFSET_Y }, // 左侧折线终点
      // Rest of the nodes - normal offset
      { id: 5, x: 13.06 * SCALE + OFFSET_X, y: 7.63 * SCALE + OFFSET_Y }, // 中部-垂直线顶端 (连着 9.1m)
      { id: 6, x: 13.06 * SCALE + OFFSET_X, y: 11.64 * SCALE + OFFSET_Y }, // 中部-垂直线底端
      { id: 7, x: 17.89 * SCALE + OFFSET_X, y: 12.69 * SCALE + OFFSET_Y }, // 中部-主根组 (5.0m 线顶端)
      { id: 8, x: 17.89 * SCALE + OFFSET_X, y: 18.13 * SCALE + OFFSET_Y }, // 中部-5.0m 线底端
      { id: 9, x: 18.15 * SCALE + OFFSET_X, y: 1.34 * SCALE + OFFSET_Y }, // 顶部三角形-上
      { id: 10, x: 18.91 * SCALE + OFFSET_X, y: 3.05 * SCALE + OFFSET_Y }, // 顶部三角形-右
      { id: 11, x: 21.85 * SCALE + OFFSET_X, y: 3.05 * SCALE + OFFSET_Y }, // 顶部三角形-下
      { id: 12, x: 22.89 * SCALE + OFFSET_X, y: 22.52 * SCALE + OFFSET_Y }, // 底部尖端 (1.8m 线末端)
      { id: 13, x: 24.81 * SCALE + OFFSET_X, y: 10.11 * SCALE + OFFSET_Y }, // 右侧主枢纽 (连接 4.2m/2.2m/6.0m)
      { id: 14, x: 27.65 * SCALE + OFFSET_X, y: 12.12 * SCALE + OFFSET_Y }, // 右侧网络-上分叉点
      { id: 15, x: 27.65 * SCALE + OFFSET_X, y: 17.94 * SCALE + OFFSET_Y }, // 底部环-右下角 (连接 1.5m 小尾巴)
      { id: 16, x: 29.14 * SCALE + OFFSET_X, y: 19.0 * SCALE + OFFSET_Y }, // 1.5m小尾巴末端 (之前可能遗了这个)
      { id: 17, x: 29.17 * SCALE + OFFSET_X, y: 6.87 * SCALE + OFFSET_Y }, // 右侧网络-顶点 (6.0m 线末端)
      { id: 18, x: 29.17 * SCALE + OFFSET_X, y: 10.11 * SCALE + OFFSET_Y }, // 右侧网络-中点 (5.4m 线末端)
      { id: 19, x: 32.99 * SCALE + OFFSET_X, y: 13.93 * SCALE + OFFSET_Y }, // 最右侧端点 (4.9m 线末端)
    ];
  ...
  useEffect(() => {
    ...
    // Add static edges (if not closed)
    edges.forEach((edge) => {
  ...
  const drawNetwork = useCallback(() => {
    ...
    edges.forEach((edge) => {
      ...
      ctx.moveTo(fromNode.x, fromNode.y);
      ctx.lineTo(toNode.x, toNode.y);
      ctx.strokeStyle = isSelected ? '#FFA2A2' : '#525252'; // Red if selected, gray otherwise
    ...
    if (
      transaction &&
      transaction.isDirectPath &&
      transaction.path.length === 2
    ) {
    ...
    nodes.forEach((node) => {
      ...
      const isInTransactionPath =
        transaction && transaction.path.includes(node.id);
      ...
      if (isStartNode) {
        ...
        nodeColor = '#ffffff'; // White for selected/hovered/transaction nodes
      }
      ...
      // Highlight ring for selected/hovered/transaction nodes (not for channel nodes)
      ...
      // Draw node ID label below the node during transaction
      if (isInTransactionPath) {
        ctx.font = '400 12px Inter';
        ctx.fillStyle = '#ffffff';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.fillText(`Node ${node.id}`, node.x, node.y + 10 + 6); // 10px gap + 6px radius
      }
    });
  ...
  }, [selectedNodes, customChannels, hoveredNodeId, transaction, nodes, isDesktop, drawNetwork, closedEdges, selectedEdge]);
  ...
  const handleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (const node of nodes) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) <= 10) {
        // Reset all green channels to gray when selecting nodes
        setCustomChannels(
    ...
    for (const edge of edges) {
      const edgeKey = `${edge.from}-${edge.to}`;
      // Skip if edge is already closed
      if (closedEdges.has(edgeKey) || closedEdges.has(`${edge.to}-${edge.from}`)) {
        continue;
      }
      ...
      if (dot >= 0 && dot <= 1) {
        ...
        if (dist <= 10) {
          ...
          if (selectedEdge?.from === edge.from && selectedEdge?.to === edge.to) {
            ...
            // Select this edge and deselect any custom channel
            setSelectedEdge(edge);
            setSelectedChannel(null);
            // Reset all custom channels (both red and green) back to gray
            setCustomChannels(
      ...
      if (dot >= 0 && dot <= 1) {
        ...
        if (dist <= 10) {
          ...
          if (
            ...
            // Select this channel and deselect any static edge
            setSelectedChannel(channel);
            setSelectedEdge(null);
            // Single pass: turn selected to red, all others to gray
            setCustomChannels(
              customChannels.map((ch) => {
                if (ch.from === channel.from && ch.to === channel.to) {
                  return { ...ch, color: '#FFA2A2' }; // Selected → red
                } else {
                  return { ...ch, color: '#525252' }; // Others → gray
                }
              })
            );
          }
  ...
  return (
    <div className="w-full lg:h-[792px] bg-layer-01 border border-invisible flex">
      ...
      <div className="hidden lg:flex w-[320px] border border-invisible flex-col">
        ...
        {/* Content Area */}
        <div
          className="flex-1 py-0 px-[16px] flex flex-col"
          style={{ gap: '20px' }}
        >
            ...
            <button
              onClick={() => {
                ...
                if (selectedNodes.length === 2) {
                  ...
                  // Clear green from all existing channels and add new green channel
                  const updatedChannels = customChannels.map(ch => ({
                    ...ch,
                    color: '#525252' // Set old channels to gray
                  }));
                  setCustomChannels([...updatedChannels, newChannel]);
                  setSelectedNodes([]); // Clear node selection
              ...
              className={`w-full h-[44px] py-[13px] px-md border flex items-center justify-center gap-sm transition-all ${
                selectedMode === 'open' ? 'bg-layer-03' : 'bg-layer-02'
              } ${
                selectedNodes.length !== 2
                  ? 'border-white/30 opacity-50 cursor-not-allowed'
                  : 'border-invisible cursor-pointer'
              }`}
            >
              <Image
                src="/plus2.svg"
          ...
          <div className="hidden lg:flex flex-col gap-sm">
            ...
            <button
              onClick={() => {
                ...
                // Close channel or edge when selected
                if (selectedChannel) {
          ...
          <button
            onClick={() => {
              ...
              // Reset all custom channels to gray and clear selections
              setCustomChannels(
                customChannels.map((ch) => ({ ...ch, color: '#525252' }))
              );
              setSelectedChannel(null);
              setSelectedEdge(null);
              ...
              while (attempts < maxAttempts) {
                ...
                sender = shuffled[0].id;
                receiver = shuffled[1].id;

                // Find path
                path = findShortestPath(sender, receiver);

                // Check if path exists and has at least 3 nodes
                if (path.length >= 3) {
                  break;
                }
              ...
              // If no valid path found after max attempts, don't start transaction
              if (path.length < 3) {
                return;
              }
              ...
              setTransaction({
                ...
                hopDuration: 500,
                isDirectPath: false, // Since we have at least 3 nodes, it's not direct
              });
            }}
            disabled={isTransacting}
            className={`w-full h-[44px] lg:mt-md py-[13px] px-md border border-white flex items-center justify-center hover-border-bright transition-all bg-layer-02 ${
      ...
      <div className="flex flex-col lg:flex-1">
        ...
        <div className="border-b border-invisible flex flex-col lg:flex-1">
          ...
          <div className="relative bg-layer-01 flex flex-col overflow-y-auto flex-1">
            <canvas
              ref={canvasRef}
              width={1200}
              height={614}
            ...
            <div
              className={`lg:hidden px-lg flex flex-col gap-sm bg-layer-01 ${
        ...
        <div className="hidden lg:flex flex-col">
          ...
          <div className="h-[80px] relative bg-layer-01 overflow-hidden border-b border-invisible">
        ...
        <div className="lg:hidden border-t border-invisible p-md flex flex-col gap-md">
          ...
          <div className="flex flex-col md:grid md:grid-cols-2 gap-sm md:gap-md">
            ...
            <div className="flex md:justify-between md:gap-sm">
              <div className="w-[90px] md:w-auto text-body3 text-tertiary">
                Nodes
              </div>
