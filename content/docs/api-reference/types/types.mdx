...
* `Stale` - The channel state may be outdated after a database restore. The node must passively audit the channel with its peer before resuming normal operation.
***
...
* `payment_preimage` - `Option<[Hash256](#type-hash256)>`, The preimage learned from a successful payment attempt.
