...
See the [API Reference](/docs/build/api-reference) for details.
...
Storage migration is part of the shared Fiber node startup path in current releases. Native `fnn` runs the unified migration system from [`crates/fiber-store`](https://github.com/nervosnetwork/fiber/tree/main/crates/fiber-store) when it opens a filesystem-backed store. The WASM runtime also enters the same migration path when `fiber-wasm` opens its browser-backed store before node services start.

The standalone `fnn-migrate` binary is a legacy tool kept under [`migrate_archive`](https://github.com/nervosnetwork/fiber/tree/main/migrate_archive). Use it only for databases older than the unified migration epoch, such as the v0.8.x upgrade path described by the Fiber repository. For current native stores, back up the data directory, start `fnn`, and let the built-in migration system run.
...
| Control a node from my backend | [HTTP RPC](/docs/build/api-reference) |
| Build a browser wallet | [fiber-js](/docs/build/fiber-js) |
| Add payments to my web game | [fiber-js](/docs/build/fiber-js) or [HTTP RPC](/docs/build/api-reference) |
