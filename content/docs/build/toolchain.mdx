...
### Community application SDKs

Community SDKs build higher-level application workflows on top of Fiber RPC or
`fiber-js`. They are maintained independently from the official Fiber
repository, so their supported Fiber versions and maintenance policies can
differ from the core runtime.

- [`fiber-pay`](/docs/build/community-sdk/fiber-pay) combines typed RPC clients,
  a browser WASM and passkey layer, React components, and CLI automation.
- [`fiber-checkout`](/docs/build/community-sdk/fiber-checkout) focuses on a
  reusable React checkout flow for creating invoices and observing payment
  status.

See [Community SDK](/docs/build/community-sdk) for the current selection policy,
...
[fnn-cli](https://github.com/nervosnetwork/fiber/tree/v0.9.1/crates/fiber-cli) is the official command-line tool for managing a Fiber node. It supports both one-shot commands and an interactive REPL with tab completion.

### Storage Migration

Storage version checking is part of the shared Fiber node startup path. Native `fnn`, `fiber-wasm`, and `fiber-ffi` all call the store opening path from [`crates/fiber-lib`](https://github.com/nervosnetwork/fiber/tree/v0.9.1/crates/fiber-lib), which initializes the version for a new store and refuses to open an incompatible or older store that needs migration.

Starting from v0.9.0, migration is built into the `fnn` binary and runs automatically on startup when needed (with user confirmation). The standalone `fnn-migrate` binary, previously built from the [`migrate`](https://github.com/nervosnetwork/fiber/tree/v0.8.0/migrate) directory, was deprecated and removed in v0.9.0. For databases created before v0.8.x (database version older than `20260302100001`), you must first upgrade using the v0.8.x `fnn-migrate` tool before starting the current `fnn` binary. See the [backup guide](/docs/operate/backup) for details.
...
| Add a browser node, passkeys, or payment components | [fiber-pay](/docs/build/community-sdk/fiber-pay) |
| Add an invoice checkout to a React app | [fiber-checkout](/docs/build/community-sdk/fiber-checkout) |
