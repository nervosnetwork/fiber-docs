...
<Callout type="warning">
  Trampoline routing is not compatible with MPP. When trampoline hops are specified, the payment is forced into single-path mode (`max_parts` is effectively 1).
</Callout>
...
| `last_error` | The most recent error message (if failed) |
| `last_error_code` | The structured error code (e.g., `TemporaryChannelFailure`) |
...
| Partial payment delivered | `Inflight` (no progress) | Some attempts failed, remaining are retrying | Wait for retries or check `last_error` for the failure reason |
