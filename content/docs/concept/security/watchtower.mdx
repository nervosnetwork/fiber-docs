...
Fiber nodes can run in two forms: the native `fnn` node and the WASM node (`fiber-wasm` / `fiber-js`). Watchtower deployment depends on which form you use.
...
Use this mode when you want a separate watchtower operator, when the built-in watchtower is disabled, or when a WASM node needs watchtower protection.
