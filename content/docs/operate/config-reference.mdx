...
| `pending_channels_number_limit` | usize | `100` | Global max pending channel openings |
...
`gossip_policy` is a config-file-only field. It is intentionally not exposed as a CLI flag or environment variable.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `gossip_policy.ban.threshold` | u32 | `100` | Ban score threshold. `0` disables this limiter. |
| `gossip_policy.ban.duration_ms` | u64 | `600000` (10 min) | Ban duration. `0` disables this limiter. |
| `gossip_policy.outbound_global.rate_bytes_per_sec` | u64 | `1024000` | Global outbound gossip byte rate |
| `gossip_policy.outbound_global.burst_bytes` | u64 | `2048000` | Global outbound gossip burst size |
| `gossip_policy.outbound_peer.rate_bytes_per_sec` | u64 | `51200` | Per-peer outbound gossip byte rate |
| `gossip_policy.outbound_peer.burst_bytes` | u64 | `102400` | Per-peer outbound gossip burst size |
| `gossip_policy.outbound_delay_queue_capacity` | usize | `1024` | Max delayed outbound gossip payloads queued per peer |
| `gossip_policy.inbound_channel_update.interval_ms` | u64 | `60000` | Inbound channel update rate-limit interval |
| `gossip_policy.inbound_channel_update.burst` | u32 | `10` | Inbound channel update burst size |
