...
| `--restore <BACKUP_PATH>` | Restore the Fiber database and node keys from a v0.9.1 timestamped backup directory, then exit. See the [native v0.9.1 restore requirements](/docs/operate/backup#restore-a-v090-online-backup). |
...
> **Tip**: Use the config files from the [Fiber v0.9.1 release](https://github.com/nervosnetwork/fiber/tree/v0.9.1/config) as starting points. They include the matching script hashes, cell deps, UDT whitelist entries, and bootnode addresses for each network.
