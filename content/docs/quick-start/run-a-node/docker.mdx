...
```bash
# From Docker Hub
docker pull nervos/fiber:0.9.1

# Or from GHCR
docker pull ghcr.io/nervosnetwork/fiber:0.9.1
```

Check [Docker Hub](https://hub.docker.com/r/nervos/fiber/tags) or [GHCR](https://github.com/nervosnetwork/fiber/pkgs/container/fiber) for available tags. For production, pin a specific version tag (for example, `0.9.1`) to avoid unexpected upgrades.
...
Exposing the RPC port to the public internet allows remote callers to control your node, including opening channels and sending payments. Keep the host-side mapping on `127.0.0.1` unless you have configured v0.9.1 RPC authentication and a secure access layer. Do not publish an unauthenticated RPC endpoint.
</Callout>
...
Then run the locally built image using `fiber:local` instead of `nervos/fiber:0.9.1`.
...
cp -a ./fiber-node ./fiber-node.backup-before-v0.9.1
```

Then start `nervos/fiber:0.9.1` with the same `/fiber` mount. v0.9.1 has migration support built into `fnn`; do not delete `fiber/store` and do not run two versions against the same directory.

<Callout type="warning" title="Databases older than v0.8.x">
v0.9.1 cannot directly migrate a pre-v0.8.x database. First use the matching v0.8.x `fnn-migrate` tool, then start v0.9.1. Follow [Backup and Restore](/docs/operate/backup) rather than guessing a migration command for valuable channel data.
</Callout>
