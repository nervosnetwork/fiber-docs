...
dependencies:
  - name: Fiber JS
    minVersion: "0.9.1"
    link: "https://www.npmjs.com/package/@nervosnetwork/fiber-js/v/0.9.1"
---
...
```bash
npm install @nervosnetwork/fiber-js@0.9.1
# or
pnpm add @nervosnetwork/fiber-js@0.9.1
```

Download the v0.9.1 Testnet config (the same template included in the native release archives) and serve it from your app:

```bash
mkdir -p public/fiber-config
curl --fail --location \
  --output public/fiber-config/testnet.yml \
  https://raw.githubusercontent.com/nervosnetwork/fiber/v0.9.1/config/testnet/config.yml
```
...
> **Expected Output:** The node initializes, connects to a configured `/ws/` or `/wss/` peer, and starts syncing. `nodeInfo()` returns your node's pubkey, `version: "0.9.1"`, and chain info; `listPeers()` should report at least one peer when the selected public node is reachable.
