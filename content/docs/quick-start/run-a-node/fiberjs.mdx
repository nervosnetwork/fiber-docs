...
dependencies:
  - name: Fiber JS
    minVersion: "0.9.0"
    link: "https://www.npmjs.com/package/@nervosnetwork/fiber-js/v/0.9.0"
---
...
```bash
npm install @nervosnetwork/fiber-js@0.9.0
# or
pnpm add @nervosnetwork/fiber-js@0.9.0
```

Download the v0.9.0 Testnet config (the same template included in the native release archives) and serve it from your app:

```bash
mkdir -p public/fiber-config
curl --fail --location \
  --output public/fiber-config/testnet.yml \
  https://raw.githubusercontent.com/nervosnetwork/fiber/v0.9.0/config/testnet/config.yml
```

The native template contains TCP bootnodes, which browsers cannot reach. Edit the existing `fiber` fields in `testnet.yml` to use browser-reachable WSS peers and avoid announcing a browser-local address. For example:
...
Keep the `chain`, `scripts`, `ckb`, and `services` sections from the downloaded template. See [Network Resources](/docs/quick-start/network-resources) for the current browser-reachable public node list.

<Callout type="warning" title="Do not use the native TCP bootnodes in a browser">
An address such as `/ip4/.../tcp/8228/p2p/...` works for native FNN but not for browser WASM. Browser peers must expose `/ws/` or `/wss/`; use WSS on HTTPS pages to avoid mixed-content blocking.
</Callout>
...
> **Expected Output:** The node initializes, connects to a configured `/ws/` or `/wss/` peer, and starts syncing. `nodeInfo()` returns your node's pubkey, `version: "0.9.0"`, and chain info; `listPeers()` should report at least one peer when the selected public node is reachable.
...
- [Build a Game with Fiber](/docs/build/gaming/simple-game) — end-to-end example with Fiber payments
