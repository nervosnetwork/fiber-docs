...
<Callout type="info">
FNN is still under active development. Before upgrading, stop the node and back up its complete data directory. v0.9.1 includes built-in migrations, backup, and restore support, but protocol and storage formats can still change between releases.
</Callout>
...
Use the native node when you need full control, persistent storage, and the ability to accept inbound channel requests from the public internet. Fiber v0.9.1 includes a guided installer that downloads the release bundle, configures the selected network, sets up keys, and generates a reusable startup script.

→ [Run a Native Node](/docs/quick-start/run-a-node/rust)

### Deploy the Native Node with Docker

If you prefer not to install the Rust toolchain or compile from source, you can deploy the same native `fnn` binary using the official Docker image. A single `docker run` command gives you a fully functional native node — it mounts a local data directory for configuration, keys, and channel state, and exposes the P2P and RPC ports just like a locally built binary.
...
Use Docker when you want a fast, reproducible native node deployment without installing build dependencies — ideal for server setups and automated environments.

→ [Run a Native Node (Docker)](/docs/quick-start/run-a-node/docker)

### WASM Node (`fiber-js`)

`fiber-js` compiles the same node logic to WebAssembly so it can run inside a browser. It stores data in IndexedDB (via a WASM worker) and is designed for client-side applications that do not want to operate a backend server.
...
Typical flow with the v0.9.1 installer:
...
→ See [WSS Configuration](https://github.com/nervosnetwork/fiber/blob/v0.9.1/docs/fiber-node-wss.md) for browser-facing deployments.
...
This pattern is covered in the [Public Nodes User Manual](https://github.com/nervosnetwork/fiber/blob/v0.9.1/docs/public-nodes.md) and is the default mental model for the WASM node.
