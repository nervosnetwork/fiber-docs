...
Download the v0.9.1 package for your platform from [GitHub Releases](https://github.com/nervosnetwork/fiber/releases/tag/v0.9.1). For example, on an Apple Silicon Mac:

```sh
curl -LO https://github.com/nervosnetwork/fiber/releases/download/v0.9.1/fnn_v0.9.1-aarch64-darwin-portable.tar.gz
tar -xzf fnn_v0.9.1-aarch64-darwin-portable.tar.gz
./fnn --version
```

The final command should report `Fiber v0.9.1`. To build instead, check out the matching tag with `git clone --branch v0.9.1 --depth 1 https://github.com/nervosnetwork/fiber.git` before running `cargo build --release`.
...
The v0.9.1 Testnet config already contains the current RUSD type script and cell dependency. Its local `auto_accept_amount` is 10 RUSD; this guide funds 20 RUSD to match the minimum accepted by the official Testnet public nodes.
