...
| `connect_peer` | Node initiating the connection | Connect to a remote peer | `pubkey`, `address` |
...
| Method | Run on | Description | Key Params |
|--------|--------|-------------|------------|
| `open_channel` | Channel initiator / funder | Open a new channel | `pubkey`, `funding_amount`, `funding_fee_rate`, `funding_udt_type_script` |
| `open_channel_with_external_funding` | Initiator whose external wallet controls `funding_lock_script` | Negotiate a channel funded by an external wallet | `pubkey`, `funding_amount`, `shutdown_script`, `funding_lock_script` |
