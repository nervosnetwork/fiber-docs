...
| Node | Trigger an online backup (`admin` module required) | Node being backed up | `fnn-cli admin backup` |
...
Restored risk state -> Stale -> Passive peer audit
```
...
| **Stale** | Restored channel state may be outdated and must be passively audited with its peer before normal operation resumes |
