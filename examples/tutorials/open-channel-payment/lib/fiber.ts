...
const fromHex = (value: string) => Uint8Array.from(value.match(/.{2}/g) ?? [], (part) => Number.parseInt(part, 16));
function profile() {
  ...
  const value = { fiberKey: toHex(crypto.getRandomValues(new Uint8Array(32))), ckbKey: toHex(crypto.getRandomValues(new Uint8Array(32))), identifier: crypto.randomUUID() };
